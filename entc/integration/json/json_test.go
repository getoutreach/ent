...
func Predicates(t *testing.T, client *ent.Client) {
	...
	r = client.User.Query().Where(func(s *sql.Selector) {
		s.Where(sqljson.ValueContainsStr(user.FieldDirs, "dev", sqljson.Path("[0]")))
	}).OnlyX(ctx)
	require.Equal(t, r.Dirs, dirs)

	r = client.User.Query().Where(func(s *sql.Selector) {
		s.Where(sqljson.ValueContainsStr(user.FieldURL, "github", sqljson.Path("Host")))
	}).OnlyX(ctx)
	require.Equal(t, r.URL, u)

	r = client.User.Query().Where(func(s *sql.Selector) {
		s.Where(sqljson.ValueHasPrefix(user.FieldT, "foo", sqljson.Path("s")))
	}).OnlyX(ctx)
	require.Equal(t, r.T.S, "foobar")

	r = client.User.Query().Where(func(s *sql.Selector) {
		s.Where(sqljson.ValueHasSuffix(user.FieldT, "bar", sqljson.Path("s")))
	}).OnlyX(ctx)
	require.Equal(t, r.T.S, "foobar")
}
